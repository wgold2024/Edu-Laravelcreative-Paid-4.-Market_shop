<template>
    <tr class="bg-gray-900">
        <td  :class="[product.parent_id ? 'bg-gray-50' : 'bg-white', 'text-center border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400']">{{ product.id }}</td>
        <td  :class="[product.parent_id ? 'bg-gray-50' : 'bg-white', 'text-center border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400']"><Link :href="route('admin.products.show', product.id)">{{ product.title }}</Link></td>
        <td  :class="[product.parent_id ? 'bg-gray-50' : 'bg-white', 'text-center border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400']">{{ product.price }}</td>
        <td  :class="[product.parent_id ? 'bg-gray-50' : 'bg-white', 'text-center border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400']">{{ product.qty }}</td>
        ...
        <td  :class="[product.parent_id ? 'bg-gray-50' : 'bg-white', 'text-center border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400']">
            <svg v-if="!product.parent_id && product.has_children" @click="getProductChildren" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 cursor-pointer mx-auto">
